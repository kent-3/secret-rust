...
// NOTE: With gRPC or gRPC-gateway, we never see MsgStoreCodeResponse, MsgExecuteContractResponse, etc.
// Instead, we get a GetTxResponse or BroadcastTxResponse, which contain `tx_response.tx.body.messages`.
// The messages are encoded as a Vec<Any>. We need to decode the `Any` values into specific Msg
// types by matching on the `type_url` field of each `Any`.
...
pub use crate::proto::secret::compute::v1beta1::AccessType;
