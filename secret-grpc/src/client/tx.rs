...
pub mod builder {
    ...
    use secretrs::compute::{MsgExecuteContract, MsgInstantiateContract, MsgStoreCode};
    ...
    pub trait Broadcast {
        ...
        async fn broadcast(self) -> Result<TxResponse<Self::Response>>;
    }
    ...
    impl<'a, Kind, From> Tx<'a, Kind, From> {
        ...
        pub async fn broadcast(
            self,
        ) -> Result<TxResponse<<Tx<'a, Kind, From> as Broadcast>::Response>>
        where
            Self: Broadcast,
        {
            <Self as Broadcast>::broadcast(self).await
        }
    }
    ...
    impl<'a, M: serde::Serialize + Send + Sync> Broadcast for Tx<'a, Initialize<M>, Account> {
        ...
        async fn broadcast(self) -> Result<TxResponse<Self::Response>> {
            ...
            if client.query_contract_label_exists(&label).await? {
                return Err(Error::ContractLabelExists(label));
            }

            let code_hash = client.query_code_hash_by_code_id(kind.code_id).await?;

            let (_, encrypted_msg) = client.encrypt_msg(&kind.msg, &code_hash, &from).await?;
        ...
        Broadcast for Tx<'a, Execute<M, R>, Account>
    {
        ...
        async fn broadcast(self) -> Result<TxResponse<Self::Response>> {
            ...
            let (nonce, encrypted_msg) = client
                .encrypt_msg(&kind.msg, kind.contract.code_hash(), &from)
                .await?;
...
impl super::Client {
        ...
        M: Msg,
    {
        ...
        let last_block_height = self.block_height().await?;
        let account_info = self.query_account_info(account).await?;
        ...
        let sign_doc = SignDoc::new(
            &body,
            &auth_info,
            &self.chain_id.parse().expect("invalid chain id"),
        ...
        // TODO - needs work to handle the BroadcastTxResponse, but this is how to broadcast a tx over gRPC!

        // pub use cosmrs::proto::cosmos::tx::v1beta1::service_client::ServiceClient as TxServiceClient;
        //
        // let tx_service_client =
        //     TxServiceClient::connect("http://grpc.testnet.secretsaturn.net:9090")
        //         .await
        //         .unwrap();
        //
        // let broadcast_tx_request = ::tonic::Request::new(BroadcastTxRequest {
        //     tx_bytes: tx_raw.to_bytes().unwrap(),
        //     mode: 2,
        // });
        //
        // let res: ::cosmrs::proto::cosmos::tx::v1beta1::BroadcastTxResponse = tx_service_client
        //     .broadcast_tx(broadcast_tx_request)
        //     .await
        //     .unwrap()
        //     .into_inner();
