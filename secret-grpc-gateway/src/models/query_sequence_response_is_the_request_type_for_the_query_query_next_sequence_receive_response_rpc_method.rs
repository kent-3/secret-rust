/*
 * Secret Network
 *
 * A REST interface for queries and transactions
 *
 * The version of the OpenAPI document: v1.12
 *
 * Generated by: https://openapi-generator.tech
 */

#[derive(Clone, Debug, PartialEq, Serialize, Deserialize)]
pub struct QuerySequenceResponseIsTheRequestTypeForTheQueryQueryNextSequenceReceiveResponseRpcMethod
{
    #[serde(
        rename = "next_sequence_receive",
        skip_serializing_if = "Option::is_none"
    )]
    pub next_sequence_receive: Option<String>,
    #[serde(rename = "proof", skip_serializing_if = "Option::is_none")]
    pub proof: Option<String>,
    #[serde(rename = "proof_height", skip_serializing_if = "Option::is_none")]
    pub proof_height: Option<Box<crate::models::HeightAtWhichTheProofWasRetrieved>>,
}

impl QuerySequenceResponseIsTheRequestTypeForTheQueryQueryNextSequenceReceiveResponseRpcMethod {
    pub fn new(
    ) -> QuerySequenceResponseIsTheRequestTypeForTheQueryQueryNextSequenceReceiveResponseRpcMethod
    {
        QuerySequenceResponseIsTheRequestTypeForTheQueryQueryNextSequenceReceiveResponseRpcMethod {
            next_sequence_receive: None,
            proof: None,
            proof_height: None,
        }
    }
}
