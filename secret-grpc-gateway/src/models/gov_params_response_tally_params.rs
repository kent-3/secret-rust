/*
 * Secret Network
 *
 * A REST interface for queries and transactions
 *
 * The version of the OpenAPI document: v1.12
 *
 * Generated by: https://openapi-generator.tech
 */

/// GovParamsResponseTallyParams : tally_params defines the parameters related to tally.

#[derive(Clone, Debug, PartialEq, Serialize, Deserialize)]
pub struct GovParamsResponseTallyParams {
    /// Minimum percentage of total stake needed to vote for a result to be  considered valid.
    #[serde(rename = "quorum", skip_serializing_if = "Option::is_none")]
    pub quorum: Option<String>,
    /// Minimum proportion of Yes votes for proposal to pass. Default value: 0.5.
    #[serde(rename = "threshold", skip_serializing_if = "Option::is_none")]
    pub threshold: Option<String>,
    /// Minimum value of Veto votes to Total votes ratio for proposal to be  vetoed. Default value: 1/3.
    #[serde(rename = "veto_threshold", skip_serializing_if = "Option::is_none")]
    pub veto_threshold: Option<String>,
    /// Minimum proportion of Yes votes for an expedited proposal to pass. Default value: 0.67.
    #[serde(
        rename = "expedited_threshold",
        skip_serializing_if = "Option::is_none"
    )]
    pub expedited_threshold: Option<String>,
}

impl GovParamsResponseTallyParams {
    /// tally_params defines the parameters related to tally.
    pub fn new() -> GovParamsResponseTallyParams {
        GovParamsResponseTallyParams {
            quorum: None,
            threshold: None,
            veto_threshold: None,
            expedited_threshold: None,
        }
    }
}
